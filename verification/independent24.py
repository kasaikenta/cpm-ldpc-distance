...
import json,time
start=time.time()
case=next(c for c in json.loads((Path(__file__).resolve().parents[1]/'joint_examples.json').read_text()) if (c['J'],c['L'])==(3,4))
P,E=case['P'],case['E'];rows=[]
assert P==24 and E==[[0,0,0,0],[0,1,2,4],[0,7,15,5]]
for row in E:
...
assert best==case['d']==24 and count==case['minimum_weight_word_count']==668
assert len(basis)==case['k'] and 4*P==case['n']
